...
def cadastrar_questoes(id_avaliacao):
    from sad.models import Questionario, Pergunta, Alternativa, Avaliacao
    from os import path
    ...
    for tipo in ['estagio','praticas','teoricas','teoricas_praticas']:
        ...
        try:
            q = Questionario.objects.filter(tipo=tipo, avaliacao=avaliacao)[0]
        except:
            # no existe esto questionario!!!
            q = Questionario(tipo=tipo, texto=tipo, avaliacao=avaliacao)
            print q
            q.save()
        # prepara as questoes
